...
(function ($) {
  ...
  function prcessData(item, index) {
  ...
  Drupal.googleCharts.drawChart = function (chartId, chartType, dataTable, googleChartOptions) {
    return function () {
      //var table = google.visualization.arrayToDataTable(JSON.parse(dataTable));
      var arr = JSON.parse(dataTable);
      var data = arr.filter(function(item){ return item[1] != "" })
      data.forEach(prcessData);
      /*
